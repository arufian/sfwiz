...
export function checkPersonaGate(persona: PersonaName, toolName: string): PersonaGateResult {
  if (READ_ONLY_PERSONAS.has(persona) && REVIEWER_BLOCKED_TOOLS.has(toolName)) {
    return {
      allowed: false,
      reason: `Persona "${persona}" is read-only; "${toolName}" is blocked.`,
    };
  }

  const isDestructiveSf = ['sf_deploy_start', 'sf_scratch_create', 'sf_assign_permset'].includes(
    toolName,
  );
  if (isDestructiveSf && !DEPLOY_PERSONAS.has(persona)) {
    return {
      allowed: false,
      reason: `Only developer/deploy-manager personas may run "${toolName}".`,
    };
  }
